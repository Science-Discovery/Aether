...
- Codex OAuth 使用订阅目录提供的名称、默认上下文、输入模态、支持档位及默认强度，用户显式配置优先；API key 模式仍使用公开目录。订阅中的 `ultra` 是 Codex 的主动多代理模式，不能直接作为 Responses 推理强度发送；Aether 尚未接入该模式，因此不自动生成此档位，也不采用该默认强度。订阅缓存保留原始有效元数据，参数单独变化也触发刷新，兼容旧的 ID 列表缓存。订阅先发布的新模型可在提供上下文和文本输入能力时加入列表；未提供的输出上限保留未知值，不把实验性最大上下文当默认上限。
