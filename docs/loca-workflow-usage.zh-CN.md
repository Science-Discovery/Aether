...
**实时进度回显**：一轮工作在单次工具调用内同步执行，耗时可能以分钟计。工作期间会话页面的 LOCA 工具气泡会实时更新（阶段清单、当前角色与尝试、调用预算进度），首行提示：**详细状态可在本项目的其他会话发送 `/loca-status` 查看**——本会话忙碌期间无法接收新输入，重复发送或点击发送按钮会**中止**当前工作。回显只包含进度（阶段/角色/预算），不包含未经审核的中间内容。
