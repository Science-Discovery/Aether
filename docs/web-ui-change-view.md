# 文件状态与 Web UI 变更视图栏实现分析

本文基于当前仓库实现，对 Web UI 中文件状态管理和“变更视图栏”相关能力做一次完整梳理。这里的“变更视图栏”并不是单一组件，而是与文件状态管理紧密关联的、会话页右侧整套“变更审查”界面，主要由以下两部分组成：
...
- `vcs` 是 Version Control System 的缩写，中文通常叫“版本控制系统”。在这个仓库里，当前实际支持的 VCS 基本就是 **Git**。
...
这个判断的作用是**性能保护**：如果用户没有打开任何变更相关的界面，前端就不会发起多余的 diff 请求。绝大多数由界面可见性驱动的按需拉取（例如首次打开、模式切换、会话切换后的后台补拉）都会先检查这个条件。

但这里有两个例外：

- **后端通过 SSE 推送的 `session.diff` 事件不受此门控约束**。当后端推送 session diff 更新时，前端的事件处理器会无条件地将数据写入缓存，无论用户当前是否在看变更视图。这样做的好处是：当用户稍后打开变更视图时，缓存里已经有最新数据，可以立刻展示，不需要等待额外的请求。
- **少数由用户显式动作触发的强制刷新也不会完全受它限制**。例如文件树操作完成后的刷新、编辑器保存后的 `session diff` 刷新，都属于“用户刚刚明确改了文件，因此应立即重算”的路径，而不是单纯的“因为界面当前可见，所以顺手拉一次”。
...
| 会话切换 | 视图可见时前端重新拉取 | 视图可见时前端重新拉取 | 缓存优先 + 延迟刷新 | 读新会话消息 |
