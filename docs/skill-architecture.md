...
└───────────┬───────────────────────────────────────┘
  ...
  （零 I/O）       │
        ...
        │                                          │
        │  buildSkillsManifest()                   │
        │  → 遍历所有 SKILL.md，记录 mtime + size  │
        └──────────┬───────────────────────────────┘
                   │
            ┌──────▼──────┐
            │  快照有效？  │  ← 与上次保存的 manifest 逐项比对
            └──────┬──────┘
              是 /   \ 否（文件被修改/新增/删除）
             /         \
            ▼           ▼
        读快照        loadSkillsFromDirs()
        反序列化      → 按顺序扫描 6 类目录
        （跳过文件    → 逐个解析 SKILL.md 内容
         内容解析）   → 同名后者覆盖前者
                          │
                          ▼
                    writeSkillsSnapshot()
...
- **手动刷新**：`skill_manage` 写入成功后调用 `clearSkillsPromptCache()`，使缓存失效
...
快照文件路径：`~/.cache/aether/.skills_prompt_snapshot.json`
...
校验逻辑：将当前所有 SKILL.md 的 mtime + size 与快照中的 manifest 逐项比对，完全一致才命中。任何文件被修改、新增、删除都会导致 miss，触发全量重扫。
...
每次 `skill_manage` 调用成功后，自动调用 `clearSkillsPromptCache()` 使 InstanceState 失效，下次调用 `Skill.available()` 时重新加载。
...
| **条件过滤时机** | 在 `build_skills_system_prompt` 中过滤，传入工具集 | 已实现 `matchesConditions`，但**目前未调用**（待激活） |
...
- **条件过滤（`matchesConditions`）**：代码已实现，`available()` 签名已还原为无工具集参数版本，需要在系统 prompt 构建时传入当前工具集并调用过滤
- **内存 LRU（Hermes Layer 1）**：Aether 用 InstanceState 替代，行为等价但不按工具集组合分 key，条件过滤激活后可考虑加入
