...
| 微信 token 过期（errcode -14） | 不变    | 保留 token，静默 1 小时后用原 token 自动重连；期间可手动重扫码恢复 |
...
- 轮询收到 errcode -14，或 iLink 在 HTTP 层返回 401/403 时，判定会话失效：**保留**本地 token（cursor 按协议清空并持久化），静默 1 小时（对齐官方 openclaw-weixin 实现，避免对过期 session 持续打流量）后用原 token 自动重连，全程**不出二维码**；
- 若服务在静默期重启，看门狗凭已保留的 token 直接恢复轮询（token 若已失效则再次进入静默-重试循环）；
- 建连阶段（会话初始化等）失败时同样**保留** token，看门狗会自动重试恢复，无需重新扫码；
- 用户在 UI 点「重试」会清除本地 token 并重新出码；「重新扫码」始终直接出码。凭据不存在时看门狗绝不自动请求二维码（避免刷码触发风控）。
