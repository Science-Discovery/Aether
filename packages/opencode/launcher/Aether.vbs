Dim fso, wsh, env, debugLog, scriptDir, exePath
Set fso = CreateObject("Scripting.FileSystemObject")
Set wsh = CreateObject("WScript.Shell")
Set env = wsh.Environment("PROCESS")
debugLog = env("AETHER_DEBUG_LOG")
...
Sub DebugLog(msg)
    ...
    If debugLog <> "" Then
        Dim ts
        Set ts = fso.OpenTextFile(debugLog, 8, True)
...
DebugLog "START | script=" & WScript.ScriptFullName & " scriptDir=" & scriptDir & " exe=" & exePath
DebugLog "ENVR | AETHER_WEB_OPEN_FALLBACK_MS=" & env("AETHER_WEB_OPEN_FALLBACK_MS") & " AETHER_DEBUG_LOG=" & debugLog

If Not fso.FileExists(exePath) Then
    DebugLog "FAIL | missing exe=" & exePath
...
stopCmd = "powershell -NoProfile -ExecutionPolicy Bypass -Command """ & _
    ...
    """"
DebugLog "STOP | running stop command"
stopCode = wsh.Run(stopCmd, 0, True)
DebugLog "STOP | exit=" & stopCode
...
DebugLog "PREP | running unblock/log trim"
psCode = wsh.Run(psCmd, 0, True)
DebugLog "PREP | exit=" & psCode

wsh.CurrentDirectory = scriptDir
DebugLog "BOOT | cwd=" & scriptDir & " cmd=cmd /c set NO_COLOR=1 && aether.exe web >> aether-log.txt 2>&1"
wsh.Run "cmd /c set NO_COLOR=1 && aether.exe web >> aether-log.txt 2>&1", 0, False
DebugLog "BOOT | command issued"
