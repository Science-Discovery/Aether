...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for Aether.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal Aether sessions
...
aether acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 aether acp
```
...
- **Terminal Integration**: Full terminal support via Aether's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | aether acp
```
...
ACP sessions map cleanly to Aether's internal session model:
