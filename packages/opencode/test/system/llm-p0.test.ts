...
const custom = process.env.OPENCODE_SYSTEM_LLM_CONFIG
const local = custom ?? path.join(dir, "providers.local.yaml")
...
function endpoint(spec: Spec, model: Model) {
  return fill(model.provider?.base_url ?? spec.base_url)
}
...
function provider(spec: Spec, model: Model, secret: string) {
  const url = endpoint(spec, model)
  return {
    name: spec.name ?? spec.id,
    npm: model.provider?.npm ?? spec.npm,
    api: url,
    env: [],
    models: {
      [model.id]: {
        ...
        ...(model.provider
          ? {
              provider: {
                ...
                ...(model.provider.base_url ? { api: fill(model.provider.base_url) } : {}),
              },
            }
          : {}),
...
async function loadSpecs() {
  ...
  if (!custom && file === sample) missing = local
