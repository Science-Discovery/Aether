...
opencode 自帶三種內建模式：**build**、**plan** 和 **docs**。您可以自訂這些模式，也可以透過 opencode 設定建立自己的模式。
...
Docs 是為文件工作設計的模式。它允許透過檔案操作來撰寫與更新文件，但預設會停用 shell 指令。

當您希望 AI 安全地編輯文件而不執行系統指令時，這個模式非常有用。
