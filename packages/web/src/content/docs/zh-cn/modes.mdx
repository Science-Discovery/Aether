...
opencode 自带三种内置模式：**build**、**plan** 和 **docs**。你可以自定义这些模式，也可以通过 opencode 配置创建自己的模式。
...
Docs 是为文档工作设计的模式。它允许进行文件操作来编写和更新文档，但默认禁用 shell 命令。

当你希望 AI 安全地编辑文档而不运行系统命令时，这个模式非常有用。
