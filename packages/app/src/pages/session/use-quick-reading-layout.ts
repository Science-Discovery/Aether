...
export function useQuickReadingLayout(options: Options) {
  const [layoutPanel, setLayoutPanel] = createStore({
  ...
  const variant = createMemo<LayoutVariant>(() =>
    getReadingLayoutVariant(options.reviewOpen(), options.fileTreeOpen()),
  )
  ...
  const pdfRatio = createMemo(() => layoutPanel.pdfRatio)
  ...
  const compositeResizeBounds = createMemo(() => {
    ...
    if (!bounds || variant() === "two-pane" || variant() === "tree") {
      return {
        min: compositePixelWidth(),
        max: compositePixelWidth(),
      }
    }
    const pdfBounds = pdfRatioBounds()
  ...
  const handleResizeWidth = (width: number) => {
    ...
    switch (layoutMode()) {
      ...
      case "review-left":
      case "review-tree-left":
        setLayoutPanel("pdfRatio", nextPdf)
        return
      case "review-right":
      case "review-tree-right": {
        const bounds = chatRatioBounds()
        ...
        setLayoutPanel("chatRatio", nextChat)
        return
      }
    }
  }
  ...
  const handleCompositeResize = (width: number) => {
    ...
    switch (layoutMode()) {
      ...
      case "review-tree-right": {
        ...
        const nextPdf = Math.min(bounds.max, Math.max(bounds.min, nextComposite - chatRatio()))
        setLayoutPanel("pdfRatio", nextPdf)
        return
      }
      default:
