...
// Row types: "create" is a special row to create a new folder
type Row = {
  absolute: string
  search: string
...
function toRow(absolute: string, home: string): Row {
  ...
  return { absolute: full, search, group: "folders" }
}
...
export function DialogNewProject(props: DialogNewProjectProps) {
  ...
  const items = async (value: string) => {
    ...
    const directoryRows = results.map((absolute) => toRow(absolute, home()))

    // Build create row from the current typed value
    if (value) {
      const resolved = resolveNewPath(value, home(), start())
      if (resolved && !directoryRows.find((r) => r.absolute === resolved.full)) {
        const createRow: Row = {
          absolute: resolved.full,
          // Use the raw typed value as search key so fuzzysort always keeps it
          search: value,
          group: "create",
          isCreate: true,
        }
        return [createRow, ...directoryRows]
      }
    }
    ...
    <Dialog title={language.t("dialog.newProject.title")}>
      <List
        ...
        sortGroupsBy={(a, b) => {
          ...
          return a.category === "create" ? -1 : 1
        }}
        groupHeader={(group) =>
          group.category === "create"
            ? language.t("dialog.newProject.createGroup")
            : language.t("dialog.newProject.parentDirectory")
        }
        ref={(r) => (list = r)}
        ...
        {(item) => {
          if (item.isCreate) {
            ...
            const display = createMemo(() => {
              const r = resolved()
              if (!r) return ""
              const parentDisplay = tildeOf(r.parent, home()) || r.parent
              return `${parentDisplay}/${r.name}`
            })
            return (
