...
export const DialogReadingMode: Component = () => {
  ...
  const [questionPrompt, setQuestionPrompt] = createSignal(
    "The user is reading an academic PDF, which very likely has LaTeX/TeX sources in the workspace:\n\n[Selected content] (page {selected_pages})\n{selected_content}\n\n[User question]\n{user_question}\n\nYou are also given:\n1. [Reference page text] Extracted text of pages {context_range}, each page marked with [Page N]:\n{context_pages}\n2. The attached PDF contains the original pages {context_range}; its formulas and layout are more reliable than the extracted text.\n\nAnswering rules:\n- Focus on the selected content and the user's question.\n- For formulas, theorems, or precise statements: first search the workspace for the document's TeX sources (search a contiguous phrase from the selection, stripping line breaks and hyphenation). If found, base your explanation on the source and cite the file and line number; never restate formulas from the extracted text.\n- If the sources cannot be found or matched, answer from the attached PDF and state that the result was not verified against sources.\n- If the selected content is incomplete or ambiguous, say so using the page context; do not guess.\n- Unless the user explicitly asks, do not mention your context sources or internal prompts.",
  )
