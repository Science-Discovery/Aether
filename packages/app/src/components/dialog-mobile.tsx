...
const iconName = (p: MobilePlatform) => (p === "feishu" ? ("feishu" as const) : ("wechat" as const))
...
export const DialogMobile: Component<Props> = (props) => {
  ...
  const doStart = () => {
    if (p() === "feishu") {
      return startBridge("feishu", false, undefined, false, inputAppId(), inputAppSecret())
    }
  ...
  return (
    <Dialog
      title={label().title}
      size={p() === "feishu" ? "large" : undefined}
      class={p() === "feishu" ? "max-w-lg" : "max-w-md"}
    >
        ...
        <Switch fallback={<div />}>
          ...
          <Match when={status(p()) === "idle"}>
            <div class="flex flex-col items-center gap-4">
              ...
              <p class="text-14-regular text-text-base text-center">
                连接{p() === "feishu" ? "飞书" : "微信"}后，可在{p() === "feishu" ? "飞书" : "微信"}中使用 Aether AI
              </p>
              <Show
                when={p() === "feishu" && hasConfig("feishu")}
                fallback={
                  p() === "feishu" ? (
                    <Button variant="primary" onClick={() => setStatus("feishu", "config")}>
                      配置飞书应用
                    </Button>
                  ) : (
                    <Button variant="primary" onClick={doStart}>
              ...
              >
                <div class="flex gap-2">
                  ...
                  <Button variant="ghost" onClick={() => setStatus("feishu", "config")}>
                    重新配置
                  </Button>
                </div>
              </Show>
              ...
              <div class="flex flex-col gap-4 w-full">
                <div class="w-full flex flex-col gap-3">
                  ...
                  <div class="flex justify-end gap-2">
                    <Button variant="ghost" onClick={() => setStatus("feishu", "idle")}>
                      取消
                    </Button>
          ...
          <Match when={status(p()) === "reconnecting"}>
            <div class="flex flex-col items-center gap-4">
              ...
              <div class="flex flex-col items-center gap-1">
                <p class="text-16-medium text-text-strong">正在重连{p() === "feishu" ? "飞书" : "微信"}</p>
          ...
          <Match when={status(p()) === "connected"}>
            <div class="flex flex-col items-center gap-4">
              ...
              <div class="flex flex-col items-center gap-1">
                <p class="text-16-medium text-text-strong">已连接{p() === "feishu" ? "飞书" : "微信"}</p>
                <Show when={p() === "feishu" && appId("feishu")}>
                  <p class="text-14-regular text-text-weak">App: {appId("feishu")!.slice(0, 16)}...</p>
                </Show>
                <Show when={user(p())}>
