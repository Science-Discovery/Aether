...
test("content search opens the file at the matched line", async ({ page, gotoSession }) => {
  await gotoSession()
  ...
  const input = dialog.getByRole("textbox").first()
  await input.fill("Customize which models appear in the model selector.")

  const items = dialog.locator('[data-slot="list-item"][data-key^="content:"]')
  let index = -1
  await expect
    .poll(
      async () => {
        const keys = await items.evaluateAll((nodes) => nodes.map((node) => node.getAttribute("data-key") ?? ""))
        index = keys.findIndex((key) => /content:packages[\\/]+app[\\/]+src[\\/]+i18n[\\/]+en\.ts:\d+:/i.test(key))
        return index >= 0
      },
      { timeout: 30_000 },
    )
  ...
  const tab = page.getByRole("tab", { name: "en.ts" })
  await expect(tab).toBeVisible()
  await tab.click()

  const selected = page.locator('[data-selected-line]').filter({
    hasText: 'Customize which models appear in the model selector.',
  })
