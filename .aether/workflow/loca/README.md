...
| `/loca-status`     | 查看阶段、调用预算和执行验收状态；工作期间可在同项目的其他会话发送（本会话忙碌） |
...
已实测 macOS 路径；Linux 分支尚未在 Linux 主机验收。Windows 暂无计算沙箱。当前不直接处理 PDF/二进制附件，也不自动安装 NumPy/SymPy 或其他科学库；应先提供可信提取的 UTF-8 资料及出处，或扩展插件内的受限执行后端。资源限制包括输出大小、文件描述符和派生进程限制，不是容器级内存配额。执行没有统一的墙钟/CPU 硬超时：运行时长由调用方 agent 判断并通过中止信号决定是否中断；角色级兜底超时按 `workflow.json` 的 `timeout` 角色映射执行（`solve` 为 `null` 即不设上限），触发后重试该步骤而不是终止整个运行。

## 配置与保存位置

`workflow.json` 配置审核数、并发数、角色调用预算、重试、修复周期、追问轮数、输入限制和检查项。默认是 3 个审核槽、并发 4、每角色最多 2 次协议尝试、每轮最多 240 次角色调用。`timeout` 是按角色的兜底上限（毫秒，`null` 表示不设上限，默认仅 `solve` 为 `null`），仅用于终止死循环或挂起的异常调用；触发后按协议重试在全新会话中重做该步骤，重试耗尽才报告失败。一次角色调用可能包含多个模型/tool turn，`calls` 不是模型 HTTP 请求数或 token 预算。单个角色的内部步数由 agent Markdown 的 `steps` 限制。
