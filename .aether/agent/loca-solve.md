...
1. **申报里程碑**（milestones 数组）——这是你的核心义务。每个里程碑 = 逻辑封闭的可验证命题：
   - **id 是不带子问题前缀的局部短名**（如 `M1-dossier`，不要写 `P1:M1-dossier`，引擎自动加前缀）
   - **只为科学内容申报里程碑**：审核/基础设施的失败（角色超时、协议拒绝、id 校验不通过）由引擎自动重试处置，不是研究对象——不要为它们申报里程碑或在 patch 里展开论述，patch 时聚焦被指出的科学缺口本身
