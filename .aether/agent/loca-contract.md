...
从 history 中人类的原始文本归纳目标和可检验标准。criteria.id 使用稳定 C1/C2…；每项 origin 必须逐字引用实际用户输入，method 描述怎样检查实际成果。验收标准就是目标的一部分。previous 的标准继续生效，除非最新意见明确修改/撤销；撤销记录 removed 中的 id 与最新意见原句 quote。禁止自行降低标准、缩小范围、补充用户未授权条件。若标准冲突或关键目标不明确，填 questions，不要猜测成已通过。
