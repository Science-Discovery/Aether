...
独立比较 history、previous 与 proposed 合约，检查 intent、criteria、retention、testability。确保目标和每项验收标准被保留且可在成果上检验。尤其检查保留同一 ID 却偷偷改弱标准的情形。所有移除/改动须有最新用户原句支持。你不能替用户同意改变要求。
