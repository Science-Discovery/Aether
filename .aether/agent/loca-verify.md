...
- **e2e**（端到端）：只依据命题、根资产与已验证前提（packet.premises）验证命题——不看也不引用原推导产物。rederivation 用**不同方法**重推（同方法重走只是 replay，不算独立验证）；limit 在已知极限/特例下检验；literature 以文献数值为基准。数值比较用 loca_execute 实际计算，给出误差与结论
