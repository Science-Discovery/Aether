...
async function uploadAll(tasks: Array<[string, Upload]>) {
  const queue = [...tasks]
  const workers = Array.from({ length: 3 }, async () => {
    for (;;) {
      const task = queue.shift()
      if (!task) return
      await put(task[0], task[1])
      console.log(`uploaded ${task[1].objectKey}`)
    }
  })
...
if (promoteDesktop) {
  ...
  await uploadAll(desktopFiles.map((name) => [join(assets, name), links.get(name)!]))
...
if (promoteWeb) {
  ...
  for (const [key, item] of Object.entries(webItems)) {
    const links = pre.platforms[key]
    if (!links || !upload(links.archive) || !upload(links.installer)) {
      fail(`Invalid web presign entry for ${key}`)
    }
    if (!links.archive.objectKey.startsWith(`${ver}/`) || !links.installer.objectKey.startsWith(`${ver}/`)) {
      fail(`Unexpected web presign object keys for ${key}`)
    }
    tasks.push([join(assets, item.archive), links.archive], [join(updates, item.installer), links.installer])
  }

  await uploadAll(tasks)
