...
description: 在 Aether (aether-dev) 中执行开发任务（修复 bug/漏洞/缺陷、实现/添加功能等）并管理全生命周期（修复+本地测试全绿+commit+review 闭环：执行 agent 本地测试全绿并 commit 即停，review 意见 PASS 后才放行；全程纯本地交付——不开 PR、不自动访问/评论 GitHub、无任何 bot 行为）。两种模式按用户措辞判定：用户强调"分配/派发"任务（到沙箱/工作区/多会话/并行 agents）→ 多沙箱模式（主 agent 直接在各沙箱 directory 下建任务会话执行任务，再在主工作区建监视子会话负责续跑与 review，主 agent 看门监视子会话）；用户未强调分配派发 → 当前会话模式（不创建子会话、不创建沙箱，在当前会话直接完成同一套标准，含 subagent review 与返工决策）。当用户提到把任务分配/派发给沙箱、sandbox、工作区、worktree、多个会话/agents 并行执行，或要求修复 bug/漏洞、实现/添加功能等开发任务时使用。
---
...
1. `git fetch upstream dev` → `git checkout -b <branch> upstream/dev` 在当前工作区切新分支（无 worktree，任务书模板里的沙箱相关表述不适用）。
2. 沿用 `references/task-prompt.md` 的全部标准节（把沙箱换成当前工作区）：准备步骤含**历史修复考古**；修复要求含**边界条件专项检查**；测试要求两层（验证测试必须、入库回归测试按需）；提交要求：规范 commit（任务源自 issue 时仅在 message 文字引用编号，**不建 issue、不访问 GitHub**），**本地测试全绿** = 最后一次 commit 后干净工作区的全量复跑通过（见模板"测试要求"）——**不开 PR、不 push、不跟踪 CI**，交付物是本地分支上的 commit。
3. **review 闭环在当前会话内**：派 `task` subagent（general 类型，只读审查，含复跑本地测试核实全绿）review 分支 commit 的 diff → 结果（PASS/FAIL 意见）回传当前会话，**由当前会话依据 review 意见决定是否放行** → FAIL 则自己在当前会话返工（同一分支追加 commit、本地测试全绿）再派新 subagent 复审，循环到 PASS；PASS 即放行收官。subagent 阻塞当前会话没有关系——单任务没有可并行的工作。
4. 完成后向用户最终汇报：任务来源（issue 编号如有）/分支/commit/改动摘要/本地测试结论/review 结论（PASS 与轮数）/考古结论。
...
主 agent（主工作区）
 ...
 │   ② 在主工作区 directory 下建监视子会话，prompt_async 派监视任务书（含任务会话ID+沙箱路径）
  └─ 看门监控：周期轮询各监视子会话是否在工作（busy=正常）
      ...
      └─ 监视汇报整体完成（review PASS）→ 读最终汇报与 review 意见决定放行，移出监控

任务会话 i（directory=沙箱，全自治干活）
  ├─ fetch → checkout -b → 修复 → commit → 干净工作区复跑全绿 → 发完成汇报后停止
  └─ 此后收到监视会话的返工指令则逐条返工（同一分支追加 commit、本地测试全绿），再停

监视子会话 i（directory=主工作区，只监视+review，不改业务代码）
  ├─ 轮询任务会话状态（GET /session/status?directory=<沙箱>）
  ├─ 任务会话停止且未完成 → prompt_async 发续跑消息到任务会话
  ├─ 任务完成且已 commit（review subagent 复跑本地测试核实真绿）→ 派 subagent review
  │   └─ FAIL → prompt_async 发返工指令给任务会话 → 等其返工完成 → 复审，循环到 PASS
  └─ PASS → 发最终汇报（含 review 意见），主 agent 决定放行后会话结束
```
...
- **监视子会话建在主工作区**：只做 API 轮询、续跑/返工指令与 review（读操作 + API/git 命令；review subagent 可在沙箱复跑测试，但禁改任何文件），不改任何业务代码。review 用 `task` subagent 在监视会话内派发，阻塞的只有监视会话自己。
...
- worktree 优先**复用已有的**（`git worktree list`）；被占用（有未提交改动）或不够时**新建**（见下）。沙箱目录上挂的分支名可能是旧任务残留，以任务书指定的**新分支**为准（从 `upstream/dev` 切）。
- 用户希望每个任务是一个可跟踪的"会话"（侧边栏可见、可随时点进去看进度；任务会话显示在沙箱行下，监视会话显示在主工作区下）。

## 派发

1. `git worktree list` 确认现有沙箱清单；`git fetch upstream dev` 确认基线；**基线差分整批只跑一次**：在涉及的 package 于 upstream/dev 跑全量测试（临时 detach worktree 跑，并记录基线 commit 号 `git rev-parse upstream/dev`，任务分支须与此同源），红名单落盘并随任务书下发给所有任务（任务会话不重跑）。
2. **分配 worktree**（每任务一个，路径记入任务书与监视任务书）：
   - 复用判定：`git -C <worktree> status --porcelain` 为空（干净）→ 复用，无论当前挂的分支是什么（任务会话会自己 `checkout -b`）。
   - 不干净或数量不够 → 新建：`git worktree add --detach <worktree父目录>/sandbox-N upstream/dev`（**N 从现有 `git worktree list` 中 sandbox 前缀的最大编号 +1 顺延**，不重号；`--detach` 不占分支名，任务会话再 checkout -b）。**用 `git worktree add`，禁止走 Aether 的 Worktree.create API**（连续创建会触发 watcher.node Bun segfault 闪退；外部 git worktree add 由 WorktreeDiscover 5s 轮询自动注册进侧边栏，是安全路径）。
...
5. 任务书必须包含（模板见 `references/task-prompt.md`）：
   - 目标 + 参考报告路径 + 沙箱路径说明（会话已建在沙箱内，cwd 即沙箱）+ **基线红名单与基线 commit 号（整批共享）**。
   - 准备步骤（`git fetch upstream dev` → `git checkout -b <branch> upstream/dev`；fetch 遇 ref lock 等 2 秒重试最多 3 次）。
   - **全新实现（禁止复用残留）**：每个沙箱对分配的任务必须基于 `upstream/dev` 新分支**从零实现**。历史残留（上批中断留下的本地/远端分支、已关闭 PR、他人旧实现）**只可读作参考**——禁止 checkout 残留分支继续写、禁止把残留提交直接充当本次交付、禁止"验证一下没问题就采纳"。汇报中可以（也应该）引用残留实现做对照说明（采纳了什么思路、为什么重做），但代码必须是本会话新写并由自己的测试验证。
   - 先读码确认问题在当前基线仍存在；不存在则停下汇报。
   - **历史修复考古**（修 bug/功能类任务必做）：只用**本地 git 历史**（`git log -S`，不访问 GitHub、不用 gh）查历史修复提交；发现修过但 bug 仍在时，必须弄清"为什么没修好/为何复现"（补丁被绕过/覆盖窗口不同/后续改动破坏），结论写进最终汇报——复现类 bug 优先从"上次为何没修住"找根因（见模板准备步骤第 4 条）。
   ...
   - **全量门禁（"本地测试全绿"的准确定义，见模板）**：touched package **全量** test+typecheck（禁单 spec 子集）；全绿 = 相对共享基线红名单（派发前统一跑一次）无新增失败；最终全量验证在最后一次 commit 后、`git status --porcelain` 全空时跑（测的就是 commit，临时测试不得混入 commit）；改 package.json 须过 `bun install --frozen-lockfile`；起后端的测试 `OPENCODE_DISABLE_MOBILE=1`、不碰本机常驻端口（详见踩坑记录）。
   - 提交流程：测试通过后规范 commit 到当前分支（commit message 文字引用 issue 编号如有；**不建 issue、不访问 GitHub**；commit 只含修复+入库回归测试），commit 后在干净工作区复跑全量 = **本地测试全绿**（见模板"提交"）。**不开 PR、不 push、不跟踪 CI**——交付物就是本地分支上的 commit。
   - **完成即停**：commit+本地测试全绿+最终汇报后停止；review 由监视子会话负责，收到返工指令（新用户消息）时逐条返工、同一分支追加 commit、本地测试全绿、再汇报（见模板"完成与停止"）。
   - 边界：只修自己的任务，不顺手修别的；最终汇报 任务来源（issue 编号如有）/分支/commit/改动/本地测试结论/考古结论。
   - 同文件相邻区域的多任务要互相注明冲突风险。
6. **监视任务书**必须包含（模板见 `references/monitor-prompt.md`）：看守对象（任务会话 ID、沙箱路径、分支、参考报告路径）；轮询循环（间隔、起跑宽限、busy/retry 处理）；停止时的核实与续跑指令写法；review 闭环（subagent 复跑本地测试核实全绿 + 只读审查分支 commit → FAIL 发返工指令 → 复审循环到 PASS）；最终汇报格式（含续跑/返工轮数与 review 意见，交主 agent 决定放行）。
7. 派发后抽查：任务会话在**各自沙箱路径**的 `/session/status` 里 busy；监视会话在**主工作区** `/session/status` 里 busy。两边都 busy 才算派发成功。
8. **禁止任何 bot 行为**：全程不自动访问 GitHub（gh 读/写、gh api、评论、建 issue、bot-claim、开 PR 一律禁止）；issue 编号只在任务书、commit message 与汇报里**文字引用**；GitHub 上的一切操作留给用户手动处理。
...
主 agent 只看门**监视子会话**（都在主工作区，一次 status 查全部）；任务会话的看护是监视子会话的职责（按 `references/monitor-prompt.md` 全自动循环：轮询→续跑→核实交付→subagent 复跑本地测试+review→返工循环到 PASS→最终汇报），主 agent 不干预其内部循环：
...
3. **监视子会话不在 status 里（停止工作）**：**不能直接当完成**——读最终汇报 `GET /session/<id>/message?directory=<主工作区>` 核实：
   - 汇报中整体已完成（commit/本地测试全绿/review PASS 齐备）→ 依据 review 意见决定放行，**移出监控，此后不再过问**。
...
4. 全部监视子会话都放行后，看门结束，派发任务整体收官（向用户总结各任务分支+commit/任务来源/review 意见与放行决定）。
...
- 会话 directory 决定 bash cwd 与文件工具默认作用域：任务会话建在沙箱后 git/文件操作直接做（无需 git -C）；**监视子会话 cwd 仍是主工作区**——只许读文件与跑 API/本地 git 命令（不访问 GitHub），禁止改主工作区与沙箱的业务文件。
- 多沙箱并行时本地测试偶发 flaky（并行沙箱同跑测试资源挤兑、vite 冷启动、重载测试超时）：先对照 dev 基线（或独跑）判断是否环境性，是则单次有限重跑并记录原因，不算回归。
- **首红重绿的 flaky 测试要记账，不能重跑过就忘**：任务会话跑与监视会话复跑**任一次**首红都算，要把失败签名（spec 名、失败断言、运行记录）记录下来；同一 spec 多次首红重绿说明它本身不稳定，会持续污染后续批次的测试判定（真回归被当成 flake 放行）。确认是测试自身问题而非本任务 diff 引入时，把证据（复现步骤与多次运行对比）写进最终汇报，由用户决定是否上报/修复，不要顺手在任务分支里修。
- 本地测试别靠"环境恰好如此"过关：起后端的测试必须 `OPENCODE_DISABLE_MOBILE=1` 且不碰本机常驻 server 端口；改 package.json 的任务必须 `bun install --frozen-lockfile` 通过——本地 node_modules/环境会掩盖的问题，到了 CI（或他人机器）才会炸。
